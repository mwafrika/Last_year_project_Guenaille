...
router.get('/', async function viewHomePage(req, res, next) {
    ...
    var hasLeave = 0;
     ...
     var chunks = await User.find({type: 'employee'}).sort({_id: -1}).exec(function getUser(err, docs) {
    ...
    //find is asynchronous function
   var myLeave = await Leave.find({applicantID: req.user._id}).sort({_id: -1}).exec(function getLeave(err, docs) {
        var hasLeave = 0;
        if (docs.length > 0) {
            hasLeave = 1;
        }
