...
router.get('/', async function viewHome(req, res, next) {
    ...
    var projectChunks = [];
   var myTask = await Project.find({employeeID: req.session.user._id}).sort({_id: -1}).exec(function getProjects(err, docs) {
        var hasProject = 0;
        if (docs.length > 0) {
            hasProject = 1;
        }
    ...
    var userChunks = [];
    var myUsers = await User.find({type: 'employee'}).sort({_id: -1}).exec(function getUser(err, docs) {
        for (var i = 0; i < docs.length; i++) {
   ...
   const myLeave = await Leave.find({applicantID: req.user._id}).sort({_id: -1}).exec(function getLeaves(err, docs) {
        var hasLeave = 0;
        if (docs.length > 0) {
            hasLeave = 1;
        }
   ...
   const myAttendance = await Attendance.find({
    ...
    console.log(projectChunks +"Haa"+ leaveChunks +"haaaa"+attendanceChunks);
